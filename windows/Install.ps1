...
$GlobalSkillRoot = Join-Path $env:USERPROFILE '.agents\skills\better-compact'
$GlobalPackageRoot = Join-Path $GlobalSkillRoot 'package'
$GlobalWindowsRoot = Join-Path $GlobalPackageRoot 'windows'
$SourceSkillPath = Join-Path $SourceRoot 'skill\SKILL.md'
if ($PSScriptRoot.TrimEnd([char]92).Equals($GlobalWindowsRoot.TrimEnd([char]92), [System.StringComparison]::OrdinalIgnoreCase)) {
    $SourceSkillPath = Join-Path $GlobalSkillRoot 'SKILL.md'
}
...
function Sync-GlobalSkill {
    $runningFromGlobalPackage = $PSScriptRoot.TrimEnd([char]92).Equals($GlobalWindowsRoot.TrimEnd([char]92), [System.StringComparison]::OrdinalIgnoreCase)

    New-Item -ItemType Directory -Force -Path $GlobalSkillRoot | Out-Null
    $globalSkillPath = Join-Path $GlobalSkillRoot 'SKILL.md'
    if (-not $SourceSkillPath.TrimEnd([char]92).Equals($globalSkillPath.TrimEnd([char]92), [System.StringComparison]::OrdinalIgnoreCase)) {
        Copy-Item -LiteralPath $SourceSkillPath -Destination $globalSkillPath -Force
    }

    $legacyNestedSkillRoot = Join-Path $GlobalPackageRoot 'skill'
    if (Test-Path -LiteralPath $legacyNestedSkillRoot -PathType Container) { Remove-Item -LiteralPath $legacyNestedSkillRoot -Recurse -Force }
    if ($runningFromGlobalPackage) { return }

    New-Item -ItemType Directory -Force -Path $GlobalPackageRoot | Out-Null
    foreach ($directory in @('windows', 'prompts')) {
        Copy-Item -LiteralPath (Join-Path $SourceRoot $directory) -Destination $GlobalPackageRoot -Recurse -Force
    }
}
...
if (-not (Test-Path -LiteralPath $SourceSkillPath -PathType Leaf)) { throw "Distribution Skill is missing: $SourceSkillPath" }
if (-not (Test-Path -LiteralPath (Join-Path $SourceRoot 'prompts\task-state.md') -PathType Leaf)) { throw "Distribution file is missing: prompts\task-state.md" }
...
if (Test-Path -LiteralPath (Join-Path $ToolRoot 'SKILL.md') -PathType Leaf) { Remove-Item -LiteralPath (Join-Path $ToolRoot 'SKILL.md') -Force }
...
Write-Host "-$WorkspaceRoot 安装成功，请到到 Codex **设置 → 钩子** 审阅并批准这三个 Hook" -ForegroundColor Green
